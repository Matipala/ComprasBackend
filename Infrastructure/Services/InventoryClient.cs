    ...
    public async Task<bool> RegisterPurchaseEntryAsync(string companyCen, string warehouseCen, string reason, List<PurchaseEntryLineDto> lines)
    {
        ...
        try
        {
            return await resilience.ExecuteAsync(async () =>
            {
                ...
                Console.WriteLine($"[Compras→Inventario] Respuesta: {(int)response.StatusCode} {response.StatusCode}");
                return response.IsSuccessStatusCode;
            });
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[Polly Fallback] RegisterPurchaseEntry - Inventario no disponible: {ex.GetType().Name} - {ex.Message}");
            return false;
        }
    }
}
